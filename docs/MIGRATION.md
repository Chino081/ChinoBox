...
| 搜索 | 关键词搜索、搜索历史、部分站点验证页 | 已迁移搜索；验证绕过不实现 |
...
| 樱花动漫 | 原源码标注正常，`.io` 注释可跳转当前 `.cc` 域名 | 已切到 `https://www.iyinghua.cc`，首页/分类/详情可用；搜索验证码不绕过 |
| AnFuns | 站点关闭 | 保留结构，禁用请求 |
| LIBVIO | 正常 | 保留 Parser |
| 在线之家 | 正常 | 保留 Parser |
| 555电影 | 不再维护 | 保留结构，禁用请求 |
| 雪落影视 | 部分异常 | 保留 Parser，不绕过验证 |
| 小宝影院 | 部分异常 | 保留 Parser，支持代理配置 |
| 纽约影院 | 部分异常 | 保留 Parser，不绕过验证 |
| ギリギリ愛 | 部分异常 | 已按当前页面结构补专用 Parser；搜索可能需要站点验证 |
